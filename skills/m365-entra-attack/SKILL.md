...
- **`ad-cs-attack`** — Hybrid-identity estates that Intune-manage devices frequently expose NDES/SCEP for certificate-based enrollment via Azure AD Application Proxy. Chain primitive: hybrid tenant confirmed → recon surfaces an internet-facing `/certsrv/mscep/mscep.dll` or `mscep_admin` endpoint tied to the same forest → `ad-cs-attack`'s external-only ESC8/challenge-password guidance applies, subject to that skill's own scope boundary.
